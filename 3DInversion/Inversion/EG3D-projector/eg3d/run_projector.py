...
@click.option('--config_path', type=str, help='configuration file', default=True, show_default=True)
def run(
        network_pkl: str,
        outdir: str,
    ...
    if c_path is not None:
        ...
        for image_list in dataset_json['labels']:
            img_fname = image_list[0].split(".")[0]
    ...
    id_image = torch.squeeze((from_im.to(device) + 1) / 2) * 255

    if latent_space_type == 'w':

        w = w_projector.project(G, config_path, c, outdir,id_image, device=device, w_avg_samples=600,num_steps = num_steps,
                                w_name=image_name)
    else:

        w = w_plus_projector.project(G, config_path, c, outdir, id_image, device=device, w_avg_samples=600, w_name=image_name, num_steps = num_steps)
        pass

    w = w.detach().cpu().numpy()
    np.save(f'{outdir}/{image_name}.npy', w)
