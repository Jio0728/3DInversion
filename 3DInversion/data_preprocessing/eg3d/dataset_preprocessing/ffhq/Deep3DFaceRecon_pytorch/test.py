...
def main(rank, opt, name='examples'):
    ...
    for i, _ in enumerate(im_path):
        ...
        #! ------ DEBUG (230219) 데이터 저장되는 위치 바꾸기 -----
        # model_save_dir = os.path.join(f"{name}/checkpoints/{opt.name}/results", 'epoch_%s_%06d'%(opt.epoch, 0))
        # model.save_mesh(os.path.join(model_save_dir, 'epoch_%s_%06d'%(opt.epoch, 0), img_name+'.obj'))
        # model.save_coeff(os.path.join(model_save_dir, 'epoch_%s_%06d'%(opt.epoch, 0), img_name+'.mat'))
        # ----- ----- ----- ----- ----- ----- -----
