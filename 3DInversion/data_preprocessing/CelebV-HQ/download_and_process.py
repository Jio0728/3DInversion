...
RT_DIR = '/home/jio/workspace'
PRJ_NAME = '3DInversion'
DATA_DIR = f'{RT_DIR}/data/{PRJ_NAME}/230215'
PRJ_DIR = os.path.join(RT_DIR, PRJ_NAME)

sys.path.append(RT_DIR)


def download(video_path, ytb_id, video_cnt, vid_id_list, pkl_path, proxy=None):
    """
    ytb_id: youtube_id
    ...
    if not os.path.exists(video_path):
        ...
        if status != 0:
            ...
            video_cnt += 1
            vid_id_list.append(ytb_id)
            with open(pkl_path, 'wb') as f:
                pickle.dump(vid_id_list, f)
...
if __name__ == '__main__':
    # json_path = f'{DATA_DIR}/celebvhq_info.json'  # json file path
    json_path = './celebvhq_info.json'
    pkl_path = f'{DATA_DIR}/celebvhq_info.pkl'  # json file path
    raw_vid_root = f'{DATA_DIR}/downloaded_celebvhq/raw/'  # download raw video path
    processed_vid_root = f'{DATA_DIR}/downloaded_celebvhq/processed/'  # processed video path
    ...
    for vid_id, save_vid_name, time, bbox in load_data(json_path):
        ...
        download(raw_vid_path, vid_id, video_cnt, vid_id_list, pkl_path, proxy)
